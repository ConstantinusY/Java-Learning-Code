...
public class Stopwatch extends AbstractStopwatch{
    ...
    public void startSW(){
        if (currentStatus == SWStatus.running){
            throw new SWAlreadyRunningException("计时器已在运行！",((System.currentTimeMillis() - sTime)/1000));
        }
        else {
    ...
    public long stopSW() throws SWTimeOverflowException {
        if(currentStatus == SWStatus.stopped){
            throw new SWAlreadyStoppedException("计时器已停止！",((System.currentTimeMillis() - sTime)/1000));
        }else{
    ...
    public void continueSW(){
        if(currentStatus == SWStatus.running){
            ...
            throw new SWAlreadyStoppedException("计时器未开始！", ((System.currentTimeMillis() - sTime) / 1000));
        } else {
    ...
    public long pauseSW() throws SWTimeOverflowException {
        if (currentStatus == SWStatus.paused){
            throw new SWAlreadyPausedException("计时器已暂停！",((System.currentTimeMillis() - sTime)/1000));
        }else if(currentStatus == SWStatus.stopped) {
            throw new SWAlreadyStoppedException("计时器未开始！", ((System.currentTimeMillis() - sTime) / 1000));
        } else {
